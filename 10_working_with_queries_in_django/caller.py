...
def find_books_by_genre_and_language(genre: str, language: str) -> str:
...
def find_authors_nationalities() -> str:
    ...
    for a in authors:
        result.append(f'{str(a)} is {a.nationality}')
...
def delete_review_by_id(id_arg: int) -> str:
    current_review = Review.objects.get(id=id_arg)
...
def filter_authors_by_nationalities(given_nationality: str) -> str:
    authors_from = Author.objects.filter(nationality=given_nationality).order_by('first_name', 'last_name')
    result = []

    for author in authors_from:
        if author.biography:
...
def filter_authors_by_birth_year(first_year, second_year):
    authors_years_between = Author.objects.filter(birth_date__year__range=(first_year, second_year)).order_by('-birth_date')
    result = [f"{a.birth_date}: {str(a)}" for a in authors_years_between]
