...
class VideoGame(models.Model):
    ...
    release_year = models.PositiveIntegerField(
        validators=[
            ...
            MinValueValidator(limit_value=1990, message='The release year must be between 1990 and 2023'),
...
class Task(models.Model):
    ...
    def search_tasks(cls, query: str) -> QuerySet:
        static_query = Q(title__icontains=query) | Q(description__icontains=query)
        return cls.objects.filter(static_query)

    @classmethod
    def recent_completed_tasks(cls, days: int) -> QuerySet:
        query = Q(is_completed=True) & Q(completion_date__gt=F('creation_date') - timedelta(days=days))
        return cls.objects.filter(query)
...
class Exercise(models.Model):
    ...
    def get_long_and_hard_exercises(cls) -> QuerySet:
        # query = Q(duration_minutes__gte=30) & Q(difficulty_level__gte=10)
